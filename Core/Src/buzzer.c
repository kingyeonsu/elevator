...
void custom_buzzer()
{
	...
	unsigned int custom_bell = G4;

	if (buzzer_indicator)
	{
		HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);
		__HAL_TIM_SET_AUTORELOAD(&htim3, divide_freq / custom_bell);
		__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, divide_freq / custom_bell / 2);  // Duty를 50%로 설정 한다.
	}
	else
	{
