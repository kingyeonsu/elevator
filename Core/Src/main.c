...
extern volatile uint8_t rx_data;   // 1. PC�?????????????????? �???????????????????�� 1byte?�� INT�?????????????????? ?��?��?���?????????????????? ???�� ?��?�� �???????????????????��
...
extern void step_motor_main_test();
extern void step_motor_forward_backward_control();
extern void step_motor_forward_backward_stop();
extern void flow_button_move();
extern void external_interrupt_step_motor_move();
...
extern volatile int buzzer_indicator;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim->Instance == TIM11)
	{
		...
		if (buzzer_indicator)
		{
			...
			if (TIM11_10ms_buzzer_counter >= 150)
			{
		...
		if (door_open)
		{
			TIM11_10ms_servo_motor_counter++;

			if (TIM11_10ms_servo_motor_counter >= 500)	// 5 sec
			{
...
//printf("EXT INT 1\n");
			...
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4, GPIO_PIN_RESET);
//			floor_state=2;
			break;
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart1, &rx_data, 1); // BT UART enable
  // 주의 : PC?? BT ?��?��?�� ?��?��?�� 별도?�� rx_data ?��?��
